--[[
    ...
    Descripci�n: Gestiona la aparici�n de los personajes personalizados.
--]]
...
-- Tabla para el "cerrojo" (debounce) para evitar bucles infinitos.
...
local function onCharacterDied(player)
	...
	if player and player:IsDescendantOf(Players) then
		isLoadingCharacter[player] = false -- Liberamos el cerrojo para permitir la reaparici�n.
...
local function onCharacterAdded(character, player)
	...
	local humanoid = newCharacter:WaitForChild("Humanoid", 5) -- A�adimos un tiempo de espera de 5s

	-- == CORRECCI�N CLAVE ==
	-- Enviamos los datos al HUD INMEDIATAMENTE despu�s de cargar el personaje.
	...
	Comm.Server:Fire(player, "InitialStatsUpdate", {
		...
		MaxEXP = 1000 -- Placeholder: Deber�as calcular esto basado en el nivel
	})
	...
	if humanoid then
		...
		warn("[CharacterHandler] No se encontr� un Humanoide en el modelo", newCharacter.Name, "despu�s de 5 segundos.")
	end
end
